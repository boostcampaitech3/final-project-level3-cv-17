...
train_pipeline = [
        ...
        dict(type='Resize', img_scale=(512,512), keep_ratio=True),# 여기에 tensor로 만드는과정이 포함 된 듯
        dict(type='RandomFlip', flip_ratio=0.5),
        dict(type='RandomRotate', prob=0.5,degree=(0,359)),
        # dict(type='RandomCrop',prob=0.5,crop_size=(384,384),cat_max_ratio=0.7),
        # dict(type='Resize', img_scale=(512,512), keep_ratio=True),
        dict(type='Albu',transforms=[
                dict(type='Compose',transforms=[
                        dict(type='CropNonEmptyMaskIfExists',height=256,width=256,p=1),
